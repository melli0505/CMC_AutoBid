...
namespace SetUnitPriceByExcel
{
    class Setting
    {
        ...
        public static void SetUnitPrice()
        {
            ...
            foreach (var file in ExcelHandling.files)
            {
            ...
            File.WriteAllText(Path.Combine(Data.work_path, "Setting_Xml.xml"), sb.ToString());
        }

        public static void GetRate()   //고정금액 및 적용비율 1, 2 저장
        {
            foreach (var bid in eleBID)
            {
                //공사 기간 저장('일' 단위)
        ...
        public static void GetData()    //xml 파일로부터 세부공사별 데이터 저장
        {
            ...
            docBID = XDocument.Load(Path.Combine(Data.work_path, "OutputDataFromBID.xml"));
