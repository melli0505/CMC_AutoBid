...
namespace SetUnitPriceByExcel
{
    class FillCostAccount
    {
        ...
        public static async void FillInvestigationCosts()
        {
            //원가 계산서 양식을 액세스 허용 구역으로 이동
            StorageFile file = await Package.Current.InstalledLocation.GetFileAsync(@"Assets\" + "세부결과_원가계산서.xlsx");
            StorageFolder copiedFolder = await Data.folder.CreateFolderAsync("세부결과_원가계산서", CreationCollisionOption.ReplaceExisting); // 원가계산서 폴더 생성
            await file.CopyAsync(copiedFolder); // 원가 계산서 양식 파일 원가계산서 폴더로 복사
            ...
            //원가 계산서 양식 불러오기
            var workbook = ExcelHandling.GetWorkbook(myfile, ".xlsx");
            ...
            costStatementPath = Path.Combine(Data.folder.Path, "원가계산서.xlsx");
        ...
        public static void FillBiddingCosts()
        {
            ...
            string costStatementPath = Path.Combine(Data.folder.Path, "원가계산서.xlsx");
            //원가계산서_세부결과 파일 불러오기
            ...
            costStatementPath = Path.Combine(Data.folder.Path, "원가계산서_세부결과.xlsx");
            ExcelHandling.WriteExcel(workbook, costStatementPath);
        }
        ...
        public static decimal GetRate(string item)
        {
            ...
            if(item.Equals("간접노무비") || item.Equals("기타경비") || item.Equals("일반관리비") ||item.Equals("공사손해보험료"))
            {
                ...
                return Math.Round(Convert.ToDecimal(Data.Bidding[item]) / Data.Investigation[item], 7) * 100;
            }
