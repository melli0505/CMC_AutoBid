...
namespace SetUnitPriceByExcel
{
    class Setting
    {
        ...
        static async void SetUnitPrice()
        {
            ...
            DirectoryInfo dir = new DirectoryInfo(copiedFolder.Path);
            ...
            File.WriteAllText(Path.Combine(Data.folder.Path, "Setting_Xml.xml"), sb.ToString());
        }
        ...
        public static void GetData()    //xml 파일로부터 세부공사별 데이터 저장
        {
            ...
            docBID = XDocument.Load(Path.Combine(Data.folder.Path, "OutputDataFromBID.xml"));
