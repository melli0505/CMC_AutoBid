...
namespace SetUnitPriceByExcel
{
    ...
    {
        static XDocument docBID;   //Result_Xml.xml
            ...
            foreach (var bid in eleBID)
            {
                if (bid.Element("C24") != null && string.Concat(bid.Element("C4").Value) == "S")
                {
                    ...
                    if (curObject.Item.Equals("제요율적용제외"))
                    {
                        ...
                        Debug.WriteLine(string.Concat(bid.Element("C9").Value) + " -> " + (Data.AdjustedExMaterial + Data.AdjustedExLabor + Data.AdjustedExExpense));
                    }
                }
            }
        }
        ...
        static void SubstitutePrice()
        {
            ...
            File.WriteAllText(Path.Combine(Data.folder.Path, "Result_Xml.xml"), sb.ToString());
        }

        public static void CreateZipFile(IEnumerable<string> files, string copiedPath)
        {
            var Zip = ZipFile.Open(Path.Combine(copiedPath, "입찰내역.zip"), ZipArchiveMode.Create);
        ...
        static async void CreateFile(decimal targetRate)
        {
            //원가 계산서 양식을 액세스 허용 구역으로 이동
            StorageFile file = await Package.Current.InstalledLocation.GetFileAsync(@"Assets\" + "입찰내역.xls");
            StorageFolder copiedFolder = await Data.folder.CreateFolderAsync("입찰내역", CreationCollisionOption.ReplaceExisting); // 입찰내역 폴더 생성
            await file.CopyAsync(copiedFolder); // 입찰 내역 양식 파일 입찰내역 폴더로 복사
            IReadOnlyList<StorageFile> costFile = await copiedFolder.GetFilesAsync();
            string myfile = costFile[0].Path;

            //최종 입찰내역 파일 세부공사별로 생성 
            CreateResultFile.Create(myfile);
            //생성된 입찰내역 파일 압축 
            string[] files = Directory.GetFiles(Data.folder.Path, "*.xls");
            CreateZipFile(files, copiedFolder.Path);
        }
        ...
        public static void Calculation(double? pRate, double? bRate)
        {
            docBID = XDocument.Load(Path.Combine(Data.folder.Path, "Setting_Xml.xml"));
            ...
            Data.PersonalRate = (decimal)pRate;
            Data.BalancedRate = (decimal)bRate;
            ...
            SubstitutePrice();  //최종 입찰내역 파일 세부공사별로 생성
            CreateFile(targetRate);
        }   
    }
}
